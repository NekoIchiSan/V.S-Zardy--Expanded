...
import openfl.Assets as OpenFlBitmap;
...
class PlayState extends MusicBeatState
{
	...
	public var reanimatedCharactersOn:String = "Off";
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'mazepart2': 	
					...
					hand = new FlxSprite(mic.x, mic.y).loadGraphic(Paths.image('cableCrowtranstion/Arm0', 'zardy'));
					hand.x = hand.x - hand.width;
					hand.y += 40;
					hand.alpha = 0;
					hand.setGraphicSize(Std.int(hand.width * 0.4));

					otherHand = new FlxSprite(hand.x,hand.y).loadGraphic(Paths.image('cableCrowtranstion/Grab', 'zardy'));
		...
		if(ClientPrefs.reanimatedCharacters == true){
			switch(SONG.song.toLowerCase()){
				...
				case "bushwhack": 
					dad = new Character(0,0, 'zardy-remasteredpart2');
			}
		}else{
		...
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 23, 400, "", 32);
		...
		songNameTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, -5, 400, SONG.song, 32);
		songNameTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.ORANGE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		songNameTxt.scrollFactor.set();
		songNameTxt.alpha = 0;
		songNameTxt.borderSize = 2;
		songNameTxt.visible = showTime;
		if(ClientPrefs.downScroll) songNameTxt.y = FlxG.height - 30;

		songMakers = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 38, 400, "", 12);
		songMakers.text = 'Song by:' + SONG.songCreator + ' & Charted by: ' + SONG.songCharter;
		songMakers.setFormat(Paths.font("vcr.ttf"), 12, FlxColor.CYAN, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		songMakers.scrollFactor.set();
		songMakers.alpha = 0;
		songMakers.borderSize = 2;
		songMakers.visible = showTime;
		if(ClientPrefs.downScroll) songMakers.y = FlxG.height - 50;
		updateTime = showTime;
		add(timeTxt);
		add(songNameTxt);
		add(songMakers);
	...
	public function backgroundVideo(source:String){ // I hate the vine shit but whatever
		#if VIDEOS_ALLOWED		
	...
	function zardyInto()
	{
		...
		zardyThinge = new FlxSprite(-10,-10).loadGraphic(Paths.image('Splash', 'zardy'));
		zardyThinge.setGraphicSize(Std.int(zardyThinge.width * 1.1));
		zardyThinge.scrollFactor.set();
		zardyThinge.cameras = [camHUD];
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				{
					case 0:
						FlxG.sound.play(Paths.sound('intro3' + introSoundsSuffix, 'preload'), 0.6);
					case 1:
						...
						FlxG.sound.play(Paths.sound('intro2' + introSoundsSuffix, 'preload'), 0.6);
					case 2:
						...
						FlxG.sound.play(Paths.sound('intro1' + introSoundsSuffix, 'preload'), 0.6);
					case 3:
	...
	function startSong():Void
	{
		...
		FlxTween.tween(songMakers, {alpha: 1}, 0.5, {ease: FlxEase.circOut});
	...
	function darknessZardy(){ // makes zardy not dark but makes everything else dark
		darknessBackground.alpha = 0.90;
		darknessBackground2.alpha = 0.05;

		var chars:Array<Character> = [boyfriend, gf, dad];
		
		for (who in chars){
			//who.color = FlxColor.fromRGB(250,250,250);
			who.color = FlxColor.fromHSL(28, 0, 98, 255);
		}
	}

	function darknessClear(){
		//remove(darknessBackground);
	...
	override public function update(elapsed:Float)
	{
		...
		if (controls.PAUSE && startedCountdown && canPause && !grabbed)
		{
				...
				if(updateTime) {
					...
					if(ClientPrefs.timeBarType != 'Song Name')
						if(ClientPrefs.timeBarType == 'Time Elapsed'){
							timeTxt.text = "Current Time: " + FlxStringUtil.formatTime(secondsTotal, false);
						}else{
							timeTxt.text = "Time Remaining: " + FlxStringUtil.formatTime(secondsTotal, false);
						}
				}
			}
		...
		*/
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			else
			{
				...
				if(SONG.song.toLowerCase() == 'bushwhack'){
					backgroundVideo('vineShit');
					// making sure these are invisible
		...
		if (!cpuControlled && startedCountdown && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode))
		{
			if(!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				if (sortedNotesList.length > 0) {
					...
					if(boyfriend.stunned == true){
						// should play just the static thing lmao.
					}else{
				...
				// Haha, Funny Agrugment in code lmao 
				// 	 	                       - Hamster_Exclusive
				...
				if(boyfriend.stunned == true){
					// just in case the first part of this doesn't work this should fucking work.
					spr.playAnim('static');
				}
	...
	function cableSpawn()
		{
			...
			onUpdate: function(tw) {
				...
				{
					FlxG.sound.play(Paths.sound("bf_mic_hit","zardy"));
					health = 0;
					lastGrab = Conductor.songPosition;
		...
		/*
			switch(curStep){
				...
				case 2191: 
					FlxTween.tween(dad, {alpha: 0.8}, 0.4);
				case 2208: 
					FlxTween.tween(dad, {alpha: 0.7}, 0.4);
				case 2224: 
					FlxTween.tween(dad, {alpha: 0.6}, 0.4);
				case 2240: 
					FlxTween.tween(dad, {alpha: 0.5}, 0.4);
				case 2304: 
					FlxTween.tween(dad, {alpha: 0.4}, 0.4);
				case 2432: 
